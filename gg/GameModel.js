...
export class GameModel {
  ...
  reset() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      } while (this.isPositionTooCloseToSnakeHead(obstacle.x, obstacle.y));
  ...
  isPositionTooCloseToSnakeHead(x, y) {
    const dx = this.snakeHead.x - x;
    const dy = this.snakeHead.y - y;
    const distance = Math.sqrt(dx * dx + dy * dy);
    const safeDistance = this.SNAKE_NODE_RADIUS + this.OBSTACLE_RADIUS + 50;
