...
export class GameModel {
  constructor() {
    ...
    this.TURN_RATE = 6; // BUGFIX: 增加转向灵敏度，从 3.5 提升到 6 度/帧
  ...
  checkCollisions() {
    ...
    // 自身碰撞
    const MIN_COLLISION_INDEX = 6; // BUGFIX: 增加到6，给转弯留出更多安全空间
    const SAFE_DISTANCE = this.NODE_DISTANCE * 3; // BUGFIX: 只检测距离蛇头一定距离外的节点，避免误伤
    for (let i = MIN_COLLISION_INDEX; i < this.snake.length; i++) {
      ...
      // BUGFIX: 增加碰撞容差，使用 0.8 倍直径而不是刚好接触就判定
      if (bodyDistance < this.SNAKE_NODE_RADIUS * 1.6) {
        this.gameOver = true;
        return;
      }
    }
