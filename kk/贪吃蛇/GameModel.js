...
export class GameModel {
  constructor() {
    ...
    this.TURN_RATE = 8; // 每帧转向角度（度） // BUGFIX: 提高转向灵敏度
  ...
  reset() {
    ...
    this.snakeHead = {
      x: startX,
      y: startY,
      angle: 180, // 朝向角度（度） // BUGFIX: 初始朝左，与蛇身排列方向一致
  ...
  updateBodyNodes() {
    ...
    // BUGFIX: 立即同步蛇头位置到 this.snake[0]，确保碰撞检测数据一致性
    this.snake[0].x = this.snakeHead.x;
    this.snake[0].y = this.snakeHead.y;

    // 更新身体节点（从i=1开始，因为i=0已同步为蛇头）
  ...
  checkCollisions() {
    ...
    // 自身碰撞
    for (let i = 4; i < this.snake.length; i++) { // BUGFIX: 从i=4开始，跳过蛇头(i=0)和前3个身体节点，避免初始状态误判
      ...
      if (bodyDistance < this.SNAKE_NODE_RADIUS * 2) {
        this.gameOver = true;
        return;
      }
    }
